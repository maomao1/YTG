...
@interface YTGBasicModels(){
    NSArray *ytg_ArrProperty;
}
@end
@implementation YTGBasicModels
+(id)shInitM{
...
-(id) setDictionaryForModel :(NSDictionary *) dic{
    return [YTGBasicModels setDictionaryForModel:dic withObject:self];
}
//字典转换成对象
+(id) setDictionaryForModel :(NSDictionary *) dic withObject:(id)baseModel{
    ...
    if (![dic isKindOfClass:[NSDictionary class]]) {
        if([dic isKindOfClass:[YTGBasicModels class]])return dic;
        return baseModel;
    }
...
+(NSArray *) setArrayForModel :(NSArray *) arr {
    ...
    for (NSDictionary *dic in arr)
    {
        ...
        //        修复
        else if([dic isKindOfClass:[YTGBasicModels class]]){
...
+(NSMutableDictionary *) setModelForDictionary :(id) baseModel {
    if (![baseModel isKindOfClass:[YTGBasicModels class]]) {
...
-(NSString *)getVauleForkey:(NSString *)key{
    if (!ytg_ArrProperty) {
        ytg_ArrProperty=[[self class] getModelAllProperty];
    }
    NSString *Property=[NSString stringWithFormat:@"%@%@",Pro_Prefix,key];
    if ([ytg_ArrProperty containsObject:Property]) {
